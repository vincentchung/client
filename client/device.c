...
u_char	header_ext_length1;
u_char	header_ext_length2;	
 ...
 */
#if 1
void dump_FTD_packet(const unsigned char *packet, struct timeval ts,
...
#endif
	ftd = (struct FTD_hdr*) packet;

	//if(ftd->header_type!=0x00)
		printf("%s header_type=%x ext_length=%x msg_length=%x,%x\n",
			timestamp_string(ts),
			(ftd->header_type),
			(ftd->header_ext_length),
			(ftd->header_ext_length1),
(ftd->header_ext_length2));
}
...
int main(int argc, char *argv[])
	...
	{
		//printf("%d\n",header.caplen);
		dump_FTD_packet(packet, header.ts, header.caplen);
	}
